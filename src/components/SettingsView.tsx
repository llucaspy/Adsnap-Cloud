    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-slide-up pb-20">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                {message && (
                    <div className="px-4 py-2 bg-accent/20 border border-accent/40 text-accent text-sm font-bold rounded-xl animate-bounce">
                        {message}
                    </div>
                )}
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="glass-panel p-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-xs focus:outline-none focus:border-accent transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="pt-4 flex justify-end">
                    <button
                        type="submit"
                        disabled={saving}
                        className="bg-accent hover:bg-accent-light text-bg-primary font-bold py-3 px-8 rounded-xl transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 flex items-center gap-2"
                    >
                        {saving ? (
                            <>
...
function Toggle({ enabled, onChange }: { enabled: boolean, onChange: (v: boolean) => void }) {
    return (
        <button
            ...
            className={`w-12 h-6 rounded-full p-1 transition-colors relative ${enabled ? 'bg-accent' : 'bg-white/10'}`}
        >
        ...
        <div className="space-y-4">
                ...
                {formats.map(format => (
                    <div key={format.id} className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/10">
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-white/50">{format.width}x{format.height} • <code className="bg-black/30 px-1 rounded text-accent">{format.selector}</code></p>
                        </div>
                        <button
                            type="button"
            ...
            <div className="grid grid-cols-12 gap-2 bg-black/20 p-4 rounded-xl border border-white/5">
                ...
                <div className="col-span-4">
                    ...
                    <input
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-sm text-white font-mono text-accent"
                ...
                <div className="col-span-1 flex items-end">
                    <button
                        ...
                        className="w-full h-[30px] bg-accent hover:bg-accent-light text-bg-primary rounded-lg flex items-center justify-center disabled:opacity-50 transition-colors"
                    >
                        <Plus size={16} />
                    </button>
                </div>
