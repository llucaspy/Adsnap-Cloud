...
import { enqueueCaptureJobs } from './workerJobs'
...
export async function runCapture(campaignId: string): Promise<BrainResponse> {
    try {
        await enqueueCaptureJobs([campaignId], {
            source: 'nexus-brain',
            priority: 20,
            allowTerminalStatuses: true,
        })

        await triggerNexusWorker([campaignId])
...
export async function getAdOpsSummary(): Promise<BrainResponse> {
    try {
        ...
        return {
            success: true,
            message: `### 📊 Central de Links AdOps\n- **Total de Dashboards Cadastrados:** ${data.total}`,
            data: {
                total: data.total
            }
        }
...
export async function getCampaignBI(piOrName: string): Promise<BrainResponse> {
    try {
        ...
        if (!campaign) {
            return {
                success: false,
                message: `Não encontrei o dashboard para "${piOrName}" no Hub.`
            }
        }

        return {
            success: true,
            message: `**Dashboard Encontrado:**\n- **Campanha:** ${campaign.campaignName}\n- **Cliente:** ${campaign.client}\n- **PI:** ${campaign.pi}\n- **Link:** ${campaign.manualDashboardUrl}`,
            data: campaign
        }
    } catch (error) {
        return { success: false, message: `Erro ao buscar dashboard no Hub: ${error}` }
    }
}
